...
import { getAdminStats, getAllUsers, getUserDetails, updateUserStatus, updateUserRole, addBonus, releaseUserFunds, getReleaseHistory, getAllSettings, updateSetting, getAllPurchases, getAllRewards, getAuditLogs, } from '../services/admin.service';
...
adminRoutes.post('/users/:id/bonus', async (c) => {
    try {
        ...
        const bonus = await addBonus(c.env.DB, c.req.param('id'), amount, type, reason, adminId);
...
adminRoutes.put('/settings/:key', async (c) => {
    try {
        ...
        const setting = await updateSetting(c.env.DB, c.req.param('key'), value, adminId);
